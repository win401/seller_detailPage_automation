...
- [ ] 선택한 스타일 세트 규칙과 누적 스타일 신호를 기획/제작 입력에 전달 — **보류(다음 배치)**: 우선순위 3의 아직 미확정된 스타일 세트 레이아웃 스키마 확장과 겹칠 위험이 있어 그쪽이 정해진 뒤 진행하는 게 중복 작업을 피함.
- [ ] 검수 에이전트에 근거 부족, 이미지 슬롯 누락, 섹션 흐름, 반복, 가독성 점검 추가 — **보류(다음 배치)**: 지금 검수 프롬프트(`review.ts`)는 섹션마다 `{id,kind,headline,body,bullets}`만 받아 slots/layoutType/이미지를 아예 못 봄 — 프롬프트 입력 확장 + `issue.type` enum 확장이 필요한 독립적인 규모의 작업이라 이번 배치와 분리.
...
- [ ] 수동 수정 신호를 사용자/스타일 세트 선호 요약으로 집계해 다음 기획에 사용 — **보류(다음 배치)**: 우선순위 2에서 이미 보류한 검수 에이전트 확장과 같은 이유로 묶어서 나중에. `user_style_signals`는 여전히 쓰기 전용(집계·기획 반영 로직 없음), `AgentType`의 `"style_signal_summary"`도 여전히 미구현 스캐폴딩.
