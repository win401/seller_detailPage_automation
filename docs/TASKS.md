...
- [x] (2026-07-16) **Konva 기반 자유 편집(Figma-style) 마이그레이션 — 1단계 수직 슬라이스** 착수. 유료 Polotno SDK($899/월~) 대신 오픈소스 `react-konva`/`konva`(MIT) 채택. "intro" 섹션 1개만 대상으로 전체 파이프라인(선택→우측 속성 패널→드래그/리사이즈→텍스트 편집→undo/redo→export) 검증 완료, 나머지 18개 layoutType 전환은 의도적으로 다음 세션들로 남김.
  ...
  - **실제 겪은 버그 2건 추가 (사용자가 실제 다운로드한 ZIP을 직접 확인)**: (3) Intro 섹션 이미지가 좌우로 비정상적으로 늘어남 — `html-to-image`가 살아있는 Konva `<canvas>`를 안정적으로 캡처하지 못하는 것이 원인으로 추정(단순 비율 계산 문제가 아님, 여러 각도로 실제 캔버스 배율 확인해도 정상이었음). `src/lib/canvas-export.ts`의 `renderCanvasDataToDataUrl()`(신규)이 `canvasData`를 별도 오프스크린 Konva 스테이지로 실제 export 해상도(860px)에 맞춰 직접 렌더링해 PNG data URL을 만들고, export 캡처 직전에만 살아있는 `<Stage>`를 그 이미지를 보여주는 평범한 `<img>`로 임시 교체(`SectionCanvasKonva`의 `staticImageUrl` prop, 캡처 후 원복) — html-to-image는 일반 `<img>`는 안정적으로 처리하므로 캔버스 임베딩 문제 자체를 우회. (4) 내보낸 PNG에 에디터 자체 크롬("상세페이지 캔버스"/"smartstore·860px" 헤더 라벨, 미리보기 테두리)까지 그대로 찍힘 — `handleExport`가 캡처하던 `canvasWrapRef`가 `<SectionCanvas>`의 최상위(헤더 바 포함) 전체를 감싸고 있던, 이번 세션 이전부터 있었지만 아무도 실제 export 파일을 열어본 적이 없어 미발견 상태였던 버그. 섹션 목록만 감싸는 새 `sectionsRootRef`로 캡처 대상을 옮겨 해결. `tsc`/`lint` 통과, Konva 자체 `toDataURL` 결과물이 정상 비율로 나오는 것은 확인했으나 **실제 ZIP 재다운로드 후 PNG 육안 확인은 아직 못함** — 다음 작업자가 가장 먼저 할 일.
