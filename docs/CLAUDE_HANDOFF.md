...
- 현재 기본은 상세페이지 섹션/블록을 조립하는 방식이지만, "자유 좌표 편집 툴은 아니다"는 더 이상 고정 원칙이 아닙니다. 구현 난이도 때문에 배제했던 결정을 뒤집고, 문장/단어 단위 텍스트 스타일링부터 시작해 더 자유로운 편집으로 단계적으로 확장하는 것이 장기 방향입니다 (2026-07-16 결정, 자세한 배경은 `docs/TASKS.md` 우선순위 4 참고).
...
"자유 좌표 기반 Figma급 편집기"는 2026-07-16부로 이 목록에서 제외했습니다. 더 이상 영구 배제 대상이 아니라 단계적으로 검토할 확장 방향입니다 (`docs/TASKS.md` 우선순위 4 참고).
