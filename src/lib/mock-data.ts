...
// 4개 모두 detail-page-templates.ts의 3개 템플릿 패밀리(living/functional/
// wellness) 중 하나의 kind별 layoutType을 그대로 preferredLayoutByKind로 가져와
// 씀 — 이미 손으로 다듬어진(mock 렌더링 검증까지 끝난) 조합만 재사용하고, 임의로
// 새 레이아웃 조합을 지어내지 않는다(docs/TASKS.md 우선순위 3 항목 8 안전장치와
// 같은 원칙). 프리미엄 리빙/따뜻한 라이프스타일 둘 다 living 패밀리를 씀 — 13개
// kind 중 7개만 실제로 패밀리마다 갈리는 현재 데이터 특성상 자연스러운 결과이며,
// 두 세트는 색상/무드/톤/이미지 슬롯 우선순위로 구분된다.
const LIVING_PREFERRED_LAYOUT: Partial<Record<SectionKind, DetailBlockLayoutType>> = {
  intro: "top_notice_banner",
  one_line: "brand_mood_story",
  problem: "problem_hook",
  solution: "big_claim_band",
  benefit_1: "material_closeup",
  benefit_2: "feature_blue_panel",
  benefit_3: "color_lineup",
  detail: "evidence_card",
  use_scene: "care_guide",
  recommended_for: "check_point_cards",
  trust: "product_info_table",
  faq: "qa_list",
  cta: "policy_notice",
};

const FUNCTIONAL_PREFERRED_LAYOUT: Partial<Record<SectionKind, DetailBlockLayoutType>> = {
  intro: "top_notice_banner",
  one_line: "problem_hook",
  problem: "big_claim_band",
  solution: "before_after_compare",
  benefit_1: "material_closeup",
  benefit_2: "feature_blue_panel",
  benefit_3: "evidence_card",
  detail: "step_guide",
  use_scene: "comparison_table",
  recommended_for: "option_grid",
  trust: "product_info_table",
  faq: "qa_list",
  cta: "policy_notice",
};

const WELLNESS_PREFERRED_LAYOUT: Partial<Record<SectionKind, DetailBlockLayoutType>> = {
  intro: "top_notice_banner",
  one_line: "problem_hook",
  problem: "big_claim_band",
  solution: "certification_stack",
  benefit_1: "material_closeup",
  benefit_2: "feature_blue_panel",
  benefit_3: "evidence_card",
  detail: "evidence_card",
...
export const mockStyleSets: StyleSet[] = [
  {
    ...
    preferredLayoutByKind: LIVING_PREFERRED_LAYOUT,
    imageSlotPriority: ["hero", "product_solo", "material", "usage_scene", "option", "size_spec", "policy"],
    imagePositionX: 50,
    imagePositionY: 35,
    imageFit: "cover",
    imageHeight: "tall",
    ...
    letterSpacing: 0,
    lineHeight: 24,
    brandNote: "단가가 높은 리빙 카테고리용 — 감성 이미지와 소재 설명 중심",
  ...
  },
  {
    ...
    sectionVisibility: {},
    preferredLayoutByKind: LIVING_PREFERRED_LAYOUT,
    imageSlotPriority: ["hero", "usage_scene", "material", "product_solo", "option", "size_spec", "policy"],
    imagePositionX: 50,
    imagePositionY: 50,
    imageFit: "cover",
    imageHeight: "default",
    ...
    letterSpacing: 0,
    lineHeight: 22,
    brandNote: "텀블러·주방용품 등 생활 밀착 라인에 쓰는 기본 세트",
  ...
  },
  {
    ...
    defaultPlatform: "ably",
    sectionVisibility: {},
    preferredLayoutByKind: FUNCTIONAL_PREFERRED_LAYOUT,
    imageSlotPriority: ["size_spec", "material", "product_solo", "usage_scene", "option", "hero", "policy"],
    imagePositionX: 50,
    imagePositionY: 50,
    imageFit: "contain",
    imageHeight: "compact",
    ...
    letterSpacing: -0.5,
    lineHeight: 18,
    brandNote: "깔창·인솔처럼 스펙/사용법 확인이 중요한 기능성 상품용",
  ...
  },
  {
    ...
    defaultTone: "trust",
    primaryColor: "#6b8f71",
    secondaryColor: "#eef5ef",
    defaultPlatform: "smartstore",
    sectionVisibility: {},
    preferredLayoutByKind: WELLNESS_PREFERRED_LAYOUT,
    imageSlotPriority: ["material", "usage_scene", "hero", "product_solo", "size_spec", "option", "policy"],
    imagePositionX: 50,
    imagePositionY: 45,
    imageFit: "cover",
    imageHeight: "default",
    ...
    letterSpacing: 0.5,
    lineHeight: 22,
    brandNote: "베개·수면용품처럼 근거 자료와 개인차 안내가 필요한 웰니스 상품용",
